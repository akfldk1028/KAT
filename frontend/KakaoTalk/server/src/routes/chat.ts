...
// 메시지를 시크릿으로 변환하는 엔드포인트
router.put('/message/:message_id/convert-to-secret', async (req, res) => {
  const message_id = Number(req.params.message_id);
  const { secret_id }: { secret_id: string } = req.body;

  if (!message_id || !secret_id) {
    return res.status(400).json({
      data: null,
      msg: '메시지 ID와 시크릿 ID가 필요합니다.'
    });
  }

  try {
    const chatting = await Chatting.findByPk(message_id);
    if (!chatting) {
    ...
    return res.json({
      ...
      msg: '메시지가 시크릿으로 변환되었습니다.'
    });
  } catch (err: any) {
    return res.status(500).json({
      data: null,
      msg: err.message || '메시지 변환 실패'
    });
  }
});
