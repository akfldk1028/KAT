...
```json
{
  ...
  "explanation": {
    ...
    "final_reasoning": "정부 통계 + AI 패턴 분석 + 초면 + 유사 사례 → 가족 사칭 스미싱 가능성 매우 높음",
    "recommended_action": {
...
대화 이력: 0일 (초면) → 금감원 통계 92% 사칭
유사 사례: 최근 7일 내 3건

Kanana Judge 판단 (AI):
- final_risk: "DANGEROUS"
- summary: "가족 사칭 스미싱 의심 (금감원 33.7%, 초면 92%)"
- final_reasoning: "정부 통계 + AI 패턴 분석 + 초면 + 유사 사례 → 매우 높음"
```
...
3. **AI 판단 근거 명시**: AI가 왜 이렇게 판단했는지 설명 ⭐
   - "가족 호칭 + 긴급성 + 금전 요구 → 전형적 가족 사칭"
   - "가족 호칭이 있지만 금전 요구 없고 긴급성 없음"

4. **유사 사례 제시**: 최근 실제 사례와 비교
   - 최근 7일 내 3건
   - 확인된 사기 사례

5. **행동 권장**: 구체적 DO/DON'T 제시
   - DO: "기존 번호로 전화 확인"
   - DON'T: "확인 전 송금 금지"
...
| 기관 | 연도 | 내용 | 적용 |
|------|------|------|------|
| 금융감독원 | 2023 | 가족사칭 33.7%, 대출 35.2%, 초면 92% | A-1, C-1, Stage 3 |
| KISA | 2024 | 택배 65%, 청첩장 1.7만건 | B-3, A-2 |
| 경찰청 | 2022 | 기관사칭 78% | B-1 |
| 경찰청 | 2023 | 공공 19.7배↑, 로맨스/몸캠 공식 분류 | B-2, A-3, C-3 |
| 국가수사본부 | 2024 | 투자 리딩방 5,340억 | C-2 |
